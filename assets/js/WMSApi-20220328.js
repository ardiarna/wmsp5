;(function (global, factory) {
  ...
  function fetchListPalletsByAreaDetails(warehouseId, areaNo, rowNo, subplant, motifId, size, shade) {
    const url = `${BASE_URL}/stock/GetListPalletByLocation.php?mode=json&area_no=${areaNo}&warehouse_id=${warehouseId}&row_no=${rowNo}&subplant=${subplant}&motif_id=${motifId}&size=${size}&shade=${shade}`;
    return axios.get(url)
      .then(response => response.data.data)
      .catch(errorHandler)
  }
    ...
    return axios.post(url, params)
      .then(response => {
        ...
        response.data.data.forEach(motifGroup => {
          ...
          // collect block qty
          if (!refGroup.block_qty.hasOwnProperty(quality)) {
            if (withRimpil) {
              refGroup.block_qty[quality] = {normal: 0, rimpil: 0}
            } else {
              refGroup.block_qty[quality] = 0;
            }
          }
          ...
          if (withRimpil) {
            const isRimpil = motifGroup.is_rimpil;
            if (isRimpil) {
              refGroup.block_qty[quality].rimpil += motifGroup.block_qty;
            } else {
              refGroup.block_qty[quality].normal += motifGroup.block_qty;
            }
          } else {
            refGroup.block_qty[quality] += motifGroup.block_qty;
          }
        });
  ...
  const location = {
    fetchPalletsByAreaSummary, fetchPalletsByAreaDetailsSummary, fetchListPalletsByAreaDetails,
