...
function getBlockQuantityRequests(subplant, fromDate, toDate, status) {
  ...
  function getLapAging(locationsubplant, dimension, isrimpil) {
    ...
    // url += `locationsubplant=${locationsubplant}`;
    // url += `&dimension=${dimension}`;
    // url += `&isrimpil=${isrimpil}`;
    const data = { locationsubplant: locationsubplant, dimension: dimension, isrimpil: isrimpil};
    return axios.post(url, data)
      .then(response => response.data.data)
      .catch(errorHandler)
  }
  ...
  const location = {
    ...
    createBlockQuantityRequests, updateBlockQuantityRequests, cancelBlockQuantityRequests, completeBlockQuantityRequests, getSalesDanProduksiBulanan, getSalesDanProduksiBulananTipe, getLapAging
  };
  const stock = {
    getPalletInfo,
