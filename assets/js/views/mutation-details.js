(function (global, factory) {
  ...
  function openAggregateMutationDetailsWindow(windows, mutationType, subplants, dateFrom, dateTo, motifIds, colName) {
    ...
    if (mutationType === COLUMN_MAP.initial_quantity) {
      ...
      grid.setHeader(
        'SUBP.,KD. MOTIF,QLTY.,DIM.,MOTIF,TGL.,MUT.,NO. PALET,TTL. PLT.,NO. BA MUAT,NO. SJ.,QTY.',
        null,
        ['', '', '', '', '', '', '', '', '', '', STYLES.TEXT_RIGHT_ALIGN, '', '', STYLES.TEXT_RIGHT_ALIGN]
      );
      grid.setColTypes('ro,ro,ro,ro,ro,ro_date,ro,ron,ro,ro,ro,ron');
      grid.setColumnIds('subplant,motif_id,quality,motif_dimension,motif_name,mutation_date,mutation_type,pallet_no,pallet_count,mutation_id,ref_txn_id,quantity');
      grid.setInitWidths('60,0,60,60,*,120,40,130,80,120,120,80');
      grid.attachHeader([FILTERS.SELECT, '&nbsp;', FILTERS.SELECT, FILTERS.SELECT, FILTERS.TEXT, FILTERS.TEXT, FILTERS.TEXT, FILTERS.TEXT, FILTERS.NUMERIC, FILTERS.TEXT, FILTERS.TEXT, FILTERS.NUMERIC]);
      grid.setColAlign('left,left,left,left,left,left,left,left,right,left,left,right');
      grid.setColSorting('str,str,str,str,str,str,str,str,int,str,str,int');
      grid.attachFooter(['&nbsp;', '&nbsp;', 'Total', gridUtils.spans.COLUMN, gridUtils.spans.COLUMN, '&nbsp;', '&nbsp;', '&nbsp;', gridUtils.reducers.STATISTICS_TOTAL, '&nbsp;', '&nbsp;', gridUtils.reducers.STATISTICS_TOTAL],
        ['', '', STYLES.TEXT_BOLD, '', '', '', '', '', STYLES.TEXT_BOLD + STYLES.TEXT_RIGHT_ALIGN, '', '', STYLES.TEXT_BOLD + STYLES.TEXT_RIGHT_ALIGN]);
    } else {
      grid.setHeader(
